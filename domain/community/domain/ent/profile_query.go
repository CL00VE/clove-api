...
func (pq *ProfileQuery) QueryComments() *CommentQuery {
...
func (pq *ProfileQuery) WithComments(opts ...func(*CommentQuery)) *ProfileQuery {
	query := (&CommentClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProfileQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*Profile, init func(*Profile), assign func(*Profile, *Post)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(post.FieldProfileID)
	}
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "profileID" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProfileQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Profile, init func(*Profile), assign func(*Profile, *Comment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Profile)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(comment.FieldProfileID)
	}
