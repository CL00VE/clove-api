...
import (
	...
	"clove-api/domain/community/domain/ent/migrate"
...
func (c *CommentClient) QueryProfile(co *Comment) *ProfileQuery {
...
func (c *ProfileClient) QueryComments(pr *Profile) *CommentQuery {
	query := (&CommentClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := pr.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Comment, Post, Profile []ent.Hook
	}
	inters struct {
		Comment, Post, Profile []ent.Interceptor
	}
)
