...
func (cq *CommentQuery) QueryProfile() *ProfileQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, comment.ProfileTable, comment.ProfileColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CommentQuery) WithProfile(opts ...func(*ProfileQuery)) *CommentQuery {
	query := (&ProfileClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) loadProfile(ctx context.Context, query *ProfileQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Profile)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "profileID" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
